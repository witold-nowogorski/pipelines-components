...
def _compiled_timeseries_pipeline_package_from_source():
    """Compile the time series training pipeline once per session to a temp YAML file."""
...
def pipeline_package_path(request):
    ...
    if request.param == "compile_from_source":
        return request.getfixturevalue("_compiled_timeseries_pipeline_package_from_source")
